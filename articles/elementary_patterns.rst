...
Not that the parentheses are important and both results need to have the
same columns.
...
Use the ``sum()`` function to check whether the percentages sum up to
100%?

Why does the ``WHERE`` clause occur twice? What happens if you leave out
one or the other.
...
The following query calculates four buckets with an equal number of data
points (quartiles):
...
Modify the query to calculate a) 10 buckets b) the median number of
births.
